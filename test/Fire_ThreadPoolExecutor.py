import cv2, torch, requests, base64, json, datetime, time, pyodbc, threading, firebase_admin, concurrent.futures
import numpy as np
...
def run_1cam(url, lockpicking_area, climbing_area, 
            ...
            fresh, frame, cnt, first_frame, second_frame, 
...
def main():
    ...
    cam_data = [cam_data[9],cam_data[10]] #ff
    ...
    while should_run_again:
        # Khởi tạo ThreadPoolExecutor với số luồng cần thiết (số camera)
        with concurrent.futures.ThreadPoolExecutor(max_workers=len(url)) as executor:
            # Sử dụng list comprehension để tạo một Future cho mỗi camera
            futures = [executor.submit(run_1cam, url[CC], lockpicking_area[CC], climbing_area[CC], 
                                    fcm_list[CC], camera_id[CC], related_camera_id[CC], 
                                    camera_name[CC], homeid[CC], lockid[CC], label_mapping[CC],
                                    fresh[CC], frame[CC], cnt[CC], first_frame[CC], second_frame[CC], 
                                    previousframe[CC], t_oldframe[CC], None_frame[CC],
                                    lastest_detected_face[CC], q_lockpicking[CC], q_climbing[CC], q_knownperson[CC],
                                    q_fire[CC], q_smoke[CC], q_fire_box[CC], q_smoke_box[CC]) for CC in range(len(url))]

            # Chờ cho ít nhất một Future kết thúc
            completed_futures, _ = concurrent.futures.wait(futures, return_when=concurrent.futures.FIRST_COMPLETED)

            # Hủy bỏ tất cả các Future còn lại
            for future in futures:
                future.cancel()

            # Kiểm tra xem có cần chạy lại không
            should_run_again = len(completed_futures) < len(url)
